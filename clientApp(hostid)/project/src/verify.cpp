...
{   
    if (this->dec_obj.has_claim("hostId")) {
        
        this->payload = this->dec_obj.payload().get_claim_value<std::string>("hostId"); 
        if (payload.find('|') != string::npos){
            this->v = split(payload , '|');
            for (int i = 0 ; i < this->v.size() ; i ++) {
                if (!this->v[i].compare(this->hostId)) {
                    return true;
                }
            }
            throw std::runtime_error("Your hostid is not registered with the license.");
        }
        else{
            if (!this->payload.compare(this->hostId)) {
                return true;
            }
            throw std::runtime_error("Your hostid is not registered with the license.");
        }
    }
    throw std::runtime_error("The key(hostId) does not exist in the payload.");
}
